...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    // Retrieve form data
    $attach_cert = !empty($_FILES['document1']['name']) ? $_FILES['document1']['name'] : $row['attach_cert'];

    // Move uploaded files to a permanent location
    if (!empty($_FILES['document1']['name'])) {
        move_uploaded_file($_FILES['document1']['tmp_name'], "directorsInfo/" . $attach_cert);
    }
    

    // Update the database
    $update_query = "UPDATE certificates SET attach_cert = ? WHERE id = ?";
    
    // Prepare the query
    $stmt = mysqli_prepare($con, $update_query);
    
    // Bind parameters
    mysqli_stmt_bind_param($stmt, "si", $attach_cert, $row['id']);
...
<div class="form-container">
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" class="shift-right">
        <label for="document1">Attached Certificate:</label><br>
