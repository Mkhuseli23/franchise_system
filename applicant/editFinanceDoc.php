...
$query = mysqli_query($con, "SELECT * FROM capital_requirements");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    // Retrieve form data
    $income_sourcr_doc = !empty($_FILES['document1']['name']) ? $_FILES['document1']['name'] : $row['income_sourcr_doc'];
    $letter_intent = !empty($_FILES['document2']['name']) ? $_FILES['document2']['name'] : $row['letter_intent'];

    // Move uploaded files to a permanent location
    if (!empty($_FILES['document1']['name'])) {
        move_uploaded_file($_FILES['document1']['tmp_name'], "capitalFinc/" . $income_sourcr_doc);
    }
    if (!empty($_FILES['document2']['name'])) {
        move_uploaded_file($_FILES['document2']['tmp_name'], "capitalFinc/" . $letter_intent);
    }

    // Update the database
    $update_query = "UPDATE capital_requirements SET 
                    income_sourcr_doc = ?,
                    letter_intent = ?
                    WHERE id = ?";
    ...
    // Bind parameters
    mysqli_stmt_bind_param($stmt, "ssi", $income_sourcr_doc, $letter_intent, $row['id']);
...
<div class="form-container">
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" class="shift-right">
        <label for="document1">Income Source Proof:</label><br>
        <input type="file" id="document1" name="document1"><br>
        <label for="document2">Letter OF Intent:</label><br>
