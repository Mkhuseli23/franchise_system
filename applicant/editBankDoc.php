...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    // Retrieve form data
    $upload_statement = !empty($_FILES['document1']['name']) ? $_FILES['document1']['name'] : $row['upload_statement'];

    // Move uploaded files to a permanent location
    if (!empty($_FILES['document1']['name'])) {
        move_uploaded_file($_FILES['document1']['tmp_name'], "bankstatements/" . $upload_statement);
    }

    // Update the database
    $update_query = "UPDATE bank_details SET 
                    upload_statement = ?
                    WHERE bank_details_id = ?";
    
    // Prepare the query
    $stmt = mysqli_prepare($con, $update_query);
    
    // Bind parameters
    mysqli_stmt_bind_param($stmt, "si", $upload_statement, $row['bank_details_id']);
...
<div class="form-container">
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" class="shift-right">
        <label for="document1">Bank Statement:</label><br>
